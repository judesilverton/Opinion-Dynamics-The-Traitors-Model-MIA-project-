...
#Opinion dynamics model: Chapter 4, Figure 9 NON BETRAYAL
...
HKmodel <- function(m, maxtime, epsilon, k, alpha, truth, truth2, lie, lie2){
...
for(n in 1:10){
  ...
  alpha_u <- (1/20)*u 
for(l in 1:iters){
  res <- HKmodel(10, 50, eps_n, 4, alpha_u, traitor, traitor2, lie2, lie)
  votes <- Mode(res)
...
write.csv(max, "~/Opinion dynamics/Figure 9 NON BETRAYAL.csv")
...
pdf("Figure 9 NON BETRAYAL.pdf", width =8, height = 6)
