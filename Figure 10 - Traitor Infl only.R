...
#Opinion dynamics model: Chapter 4, Figure 10
...
#Influence level for traitor and truth seeker will be varied, all others set to 1
...
write.csv(max, "~/Opinion dynamics/Figure 10.csv")
...
pdf("Figure 10.pdf", width =9, height = 6)
ggplot(r_df, aes(x, y, fill= as.factor(layer))) + 
  geom_tile(color = "black") + scale_fill_manual(breaks=c(0,1,2,3), labels=c("","Traitor","Truth seeker","Other"),
                                                 values=c("white","darkred","slateblue3","palegreen1")) + labs(x = "Influence level of traitor\n and truth seeker", y = expression(epsilon)) +
